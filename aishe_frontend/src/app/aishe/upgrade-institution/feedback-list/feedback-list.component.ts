...
export class FeedbackListComponent implements OnInit {
    ...
    this.authService.getFeedBackList(aisheCode, currentSurveyYear).subscribe(res => {
      if (res.status == 200) {
        ...
        this.feedbackList.map((e: any) => {
          ...
          // res.dateTime=res.dateTime.split(' ')[0];
          
        })
        this.statusValue=this.filterType;
        this.feedbackList=this.HEIList;
  ...
  tabSelected(event: any) {
    if (event.index === 0) {
      this.feedbackList=[];
      // this.filterType=2;//unreAD
      this.showList(1);
      // this.getFeedBack();  
    } else if (event.index === 1) {
      // this.HEIList=[];
      // this.filterType=1;//read
      this.showList(2);
      // this.getFeedBack();
