...
@Component({
  selector: 'app-report103c',
  templateUrl: './report103c.component.html',
  styleUrls: ['./report103c.component.scss']
})
export class Report103cComponent implements OnInit, ReportChildComponent {
  @Input("reportInfoObj")
  reportInfoObj: any;
  ...
  exportAsXLSX(): void {
    ...
    this.reportFilterForm.value.exportType="EXCEL";
      this.getService.getReport103C(this.reportFilterForm.value,this.inputObj).subscribe((res:any) => {
  ...
  getReportDataTable(): void {
    ...
    this.getService.getReport103C(this.reportFilterForm.value,this.inputObj).pipe(
      finalize(() => {
