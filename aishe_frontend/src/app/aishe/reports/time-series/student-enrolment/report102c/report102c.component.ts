...
@Component({
  selector: 'app-report102c',
  templateUrl: './report102c.component.html',
  styleUrls: ['./report102c.component.scss']
})
export class Report102cComponent implements OnInit , ReportChildComponent {
  @Input("reportInfoObj")
  reportInfoObj: any;
  ...
  exportAsXLSX(): void {
    ...
    this.getService.getReport102C(this.reportFilterForm.value,this.inputObj).subscribe((res:any) => {
        let byteArrays = res.byteData;
        utility.downloadAsExcel(byteArrays,this.reportInfoObj.reportNumber);
  });
  }
  ngOnInit(): void {
    console.log("inside reports102C")
  ...
  getReportDataTable(): void {
    ...
    this.getService.getReport102C(this.reportFilterForm.value,this.inputObj).pipe(
      finalize(() => {
