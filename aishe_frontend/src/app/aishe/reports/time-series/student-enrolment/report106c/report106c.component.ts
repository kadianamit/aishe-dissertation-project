...
@Component({
  selector: 'app-report106c',
  templateUrl: './report106c.component.html',
  styleUrls: ['./report106c.component.scss']
})
export class Report106cComponent implements OnInit , ReportChildComponent {
  @Input("reportInfoObj")
  reportInfoObj: any;
  ...
  exportAsXLSX(): void {
    ...
    this.getService.getReport106C(this.reportFilterForm.value,this.inputObj).subscribe((res:any) => {
        let byteArrays = res.byteData;
        utility.downloadAsExcel(byteArrays,this.reportInfoObj.reportNumber);
  });
  }
  ngOnInit(): void {
    console.log("inside reports106b")
  ...
  getReportDataTable(): void {
    ...
    this.getService.getReport106C(this.reportFilterForm.value,this.inputObj).pipe(
      finalize(() => {
