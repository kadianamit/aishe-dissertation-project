...
@Component({
  selector: 'app-report105c',
  templateUrl: './report105c.component.html',
  styleUrls: ['./report105c.component.scss']
})
export class Report105cComponent implements OnInit , ReportChildComponent {
  @Input("reportInfoObj")
  reportInfoObj: any;
  ...
  exportAsXLSX(): void {
    ...
    this.getService.getReport105C(this.reportFilterForm.value,this.inputObj).subscribe((res:any) => {
        let byteArrays = res.byteData;
        utility.downloadAsExcel(byteArrays,this.reportInfoObj.reportNumber);
  });
  }
  ngOnInit(): void {
    console.log("inside reports105c")
  ...
  getReportDataTable(): void {
    ...
    this.getService.getReport105C(this.reportFilterForm.value,this.inputObj).pipe(
      finalize(() => {
