...
@Component({
  selector: 'app-report106a',
  templateUrl: './report106a.component.html',
  styleUrls: ['./report106a.component.scss']
})
export class Report106aComponent implements OnInit , ReportChildComponent {
  @Input("reportInfoObj")
  reportInfoObj: any;
  ...
  exportAsXLSX(): void {
    ...
    this.getService.getReport106A(this.reportFilterForm.value,this.inputObj).subscribe((res:any) => {
        let byteArrays = res.byteData;
        utility.downloadAsExcel(byteArrays,this.reportInfoObj.reportNumber);
  });
  }
  ngOnInit(): void {
    console.log("inside reports106")
  ...
  getReportDataTable(): void {
    ...
    this.getService.getReport106A(this.reportFilterForm.value,this.inputObj).pipe(
      finalize(() => {
