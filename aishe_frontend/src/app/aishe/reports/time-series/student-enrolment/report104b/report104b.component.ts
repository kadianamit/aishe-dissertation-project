...
@Component({
  selector: 'app-report104b',
  templateUrl: './report104b.component.html',
  styleUrls: ['./report104b.component.scss']
})
export class Report104bComponent implements OnInit , ReportChildComponent {
  @Input("reportInfoObj")
  reportInfoObj: any;
...
splitAisheCode:any
  ...
  exportAsXLSX(): void {
    ...
    this.getService.getReport104B(this.reportFilterForm.value,this.inputObj).subscribe((res:any) => {
        let byteArrays = res.byteData;
        utility.downloadAsExcel(byteArrays,this.reportInfoObj.reportNumber);
  });
  }
  ngOnInit(): void {
    console.log("inside reports104b")
  ...
  getReportDataTable(): void {
    ...
    this.reportFilterForm.value.universityId=this.splitAisheCode;
    this.getService.getReport104B(this.reportFilterForm.value,this.inputObj).pipe(
      finalize(() => {
