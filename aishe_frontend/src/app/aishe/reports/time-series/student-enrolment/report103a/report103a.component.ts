...
@Component({
  selector: 'app-report103a',
  templateUrl: './report103a.component.html',
  styleUrls: ['./report103a.component.scss']
})
export class Report103aComponent implements OnInit , ReportChildComponent {
  @Input("reportInfoObj")
  reportInfoObj: any;
  ...
  exportAsXLSX(): void {
    ...
    this.reportFilterForm.value.exportType="EXCEL";
      this.getService.getReport103A(this.reportFilterForm.value,this.inputObj).subscribe((res:any) => {
          let byteArrays = res.byteData;
          utility.downloadAsExcel(byteArrays,this.reportInfoObj.reportNumber);
    });
  }
  ngOnInit(): void {
    console.log("inside reports103A")
  ...
  getReportDataTable(): void {
    ...
    this.getService.getReport103A(this.reportFilterForm.value,this.inputObj).pipe(
      finalize(() => {
