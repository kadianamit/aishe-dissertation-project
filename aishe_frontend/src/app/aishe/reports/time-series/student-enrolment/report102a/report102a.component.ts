...
@Component({
  selector: 'app-report102a',
  templateUrl: './report102a.component.html',
  styleUrls: ['./report102a.component.scss']
})
export class Report102aComponent implements OnInit , ReportChildComponent {
  @Input("reportInfoObj")
  reportInfoObj: any;
  ...
  exportAsXLSX(): void {
    ...
    this.getService.getReport102A(this.reportFilterForm.value,this.inputObj).subscribe((res:any) => {
        let byteArrays = res.byteData;
        utility.downloadAsExcel(byteArrays,this.reportInfoObj.reportNumber);
  });
  }
  ngOnInit(): void {
    console.log("inside reports102A")
  ...
  getReportDataTable(): void {
    ...
    this.getService.getReport102A(this.reportFilterForm.value,this.inputObj).pipe(
      finalize(() => {
