...
@Component({
  selector: 'app-report102b',
  templateUrl: './report102b.component.html',
  styleUrls: ['./report102b.component.scss']
})
export class Report102bComponent implements OnInit  , ReportChildComponent {
  @Input("reportInfoObj")
  reportInfoObj: any;
  ...
  exportAsXLSX(): void {
    ...
    this.getService.getReport102B(this.reportFilterForm.value,this.inputObj).subscribe((res:any) => {
        let byteArrays = res.byteData;
        utility.downloadAsExcel(byteArrays,this.reportInfoObj.reportNumber);
  });
  }
  ngOnInit(): void {
    console.log("inside reports102B")
  ...
  getReportDataTable(): void {
    ...
    this.getService.getReport102B(this.reportFilterForm.value,this.inputObj).pipe(
      finalize(() => {
