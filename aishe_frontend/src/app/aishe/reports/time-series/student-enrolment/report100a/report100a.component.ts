...
@Component({
  selector: 'app-report100a',
  templateUrl: './report100a.component.html',
  styleUrls: ['./report100a.component.scss']
})
export class Report100aComponent implements OnInit , ReportChildComponent {
  @Input("reportInfoObj")
  reportInfoObj: any;
  ...
  constructor(private getService:GetService ,public sharedService:SharedService) { }
    ...
    exportAsXLSX(): void {
      ...
      this.getService.getReport100A(this.reportFilterForm.value,this.inputObj).subscribe((res:any) => {
          let byteArrays = res.byteData;
          utility.downloadAsExcel(byteArrays,this.reportInfoObj.reportNumber);
    });
  }
  ngOnInit(): void {
    console.log("inside reports100")
  ...
  getReportDataTable(): void {
    ...
    this.getService.getReport100A(this.reportFilterForm.value,this.inputObj).pipe(
      finalize(() => {
...
this.reportFilterForm.value=this.reportFilterForm.getRawValue();
  ...
  handlePageChange(event: number) {
   ...
   // let fgh = parseInt(this.pageSize);
    ...
    var a = Math.ceil(this.reportTableData.length / this.pageSize);
    if (a === event) {
