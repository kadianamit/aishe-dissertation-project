...
@Component({
  selector: 'app-report105a',
  templateUrl: './report105a.component.html',
  styleUrls: ['./report105a.component.scss']
})
export class Report105aComponent implements OnInit, ReportChildComponent {
  @Input("reportInfoObj")
  reportInfoObj: any;
  ...
  exportAsXLSX(): void {
    ...
    this.getService.getReport105A(this.reportFilterForm.value,this.inputObj).subscribe((res:any) => {
        let byteArrays = res.byteData;
        utility.downloadAsExcel(byteArrays,this.reportInfoObj.reportNumber);
  });
  }
  ngOnInit(): void {
    console.log("inside reports103B")
  ...
  getReportDataTable(): void {
    ...
    this.getService.getReport105A(this.reportFilterForm.value,this.inputObj).pipe(
      finalize(() => {
