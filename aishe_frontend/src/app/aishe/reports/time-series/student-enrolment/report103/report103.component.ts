...
@Component({
  selector: 'app-report103',
  templateUrl: './report103.component.html',
  styleUrls: ['./report103.component.scss']
})
export class Report103Component implements OnInit , ReportChildComponent {
  @Input("reportInfoObj")
  reportInfoObj: any;
  ...
  exportAsXLSX(): void {
    this.reportFilterForm.value=this.reportFilterForm.getRawValue();
      ...
      this.getService.getReport103(this.reportFilterForm.value,this.inputObj).subscribe((res:any) => {
          let byteArrays = res.byteData;
          utility.downloadAsExcel(byteArrays,this.reportInfoObj.reportNumber);
    });
  }
  ngOnInit(): void {
    console.log("inside reports103")
  ...
  getReportDataTable(): void {
    ...
    this.getService.getReport103(this.reportFilterForm.value,this.inputObj).pipe(
      finalize(() => {
