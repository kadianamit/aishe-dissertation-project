...
@Component({
  selector: 'app-report107c',
  templateUrl: './report107c.component.html',
  styleUrls: ['./report107c.component.scss']
})
export class Report107cComponent implements OnInit, ReportChildComponent {
  ...
  exportAsXLSX(): void {
    ...
    this.PostService.getReport107C(this.reportFilterForm.value).subscribe((res:any) => {
  ...
  getReportDataTable(): void {
    ...
    this.PostService.getReport107C(this.reportFilterForm.value).pipe(
      finalize(() => {
  ...
  generatePDF(): void {
    ...
    this.PostService.getReport107C(this.reportFilterForm.value).subscribe(res => {
        let byteArrays = res.byteData;
