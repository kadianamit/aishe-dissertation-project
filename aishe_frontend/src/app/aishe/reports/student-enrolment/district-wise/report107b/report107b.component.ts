...
@Component({
  selector: 'app-report107b',
  templateUrl: './report107b.component.html',
  styleUrls: ['./report107b.component.scss']
})
export class Report107bComponent implements OnInit,ReportChildComponent {
  ...
  getReportDataTable(){
    ...
    this.PostService.getReport107B(this.reportFilterForm.value).pipe(
      finalize(() => {
      ...
      if (res.list.length > 0) {
        ...
        this.Showdata12=false;
        this.showPdfButton = false
        this.shareService.showError("Data is not available");
      }
    });
  }
  generatePDF(): void {
    ...
    this.PostService.getReport107B(this.reportFilterForm.value).subscribe(res => {
        let byteArrays = res.byteData;
