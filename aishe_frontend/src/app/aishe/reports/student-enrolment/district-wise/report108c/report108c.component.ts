...
@Component({
  selector: 'app-report108c',
  templateUrl: './report108c.component.html',
  styleUrls: ['./report108c.component.scss']
})
export class Report108cComponent implements OnInit,ReportChildComponent {
  ...
  exportAsXLSX(): void {
    ...
    this.ReportService.getReport108C(this.reportFilterForm.value).subscribe((res:any) => {
...
this.ReportService.getReport108C(this.reportFilterForm.value).pipe(
  finalize(() => {
  ...
  generatePDF(): void {
   ...
   this.ReportService.getReport108C(this.reportFilterForm.value).subscribe(res => {
       let byteArrays = res.byteData;
