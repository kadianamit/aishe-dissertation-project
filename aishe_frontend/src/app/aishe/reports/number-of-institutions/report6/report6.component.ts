...
export class Report6Component implements OnInit, ReportChildComponent {
  ...
  this.reportFilterForm.value = this.reportFilterForm.getRawValue();
    ...
    this.reportService.getNumberOfInstStateWiseReport(this.reportFilterForm.value).subscribe(res => {
          let byteArrays = res.byteData;
          utility.downloadAsExcel(byteArrays,this.reportInfoObj.reportNumber);
    });
  }
  ngOnInit(): void {
    console.log("inside reports6")
  }
  ...
  console.log(this.reportFilterForm.value.addressStateCode.stateCode)
    this.reportService.getnumberOfInstStateWiseReport(this.reportFilterForm.value).pipe(
      finalize(() => {
