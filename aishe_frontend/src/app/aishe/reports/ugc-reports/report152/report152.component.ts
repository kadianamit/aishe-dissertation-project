...
@Component({
  selector: 'app-report152',
  templateUrl: './report152.component.html',
  styleUrls: ['./report152.component.scss']
})
export class Report152Component implements OnInit, ReportChildComponent {
  ...
  exportAsXLSX(): void {
    ...
    this.getService.getReport152(this.reportFilterForm.value).subscribe(res => {
        let byteArrays = res.byteData;
        utility.downloadAsExcel(byteArrays,this.reportInfoObj.reportNumber);
  });
  }
  ngOnInit(): void {
    console.log("inside report152")
...
this.reportFilterForm.value=this.reportFilterForm.getRawValue();
   ...
   // this.showPdfButton = true;
    this.getService.getReport152(this.reportFilterForm.value).pipe(
      finalize(() => {
