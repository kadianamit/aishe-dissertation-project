...
@Component({
  selector: 'app-report134',
  templateUrl: './report134.component.html',
  styleUrls: ['./report134.component.scss']
})
export class Report134Component implements OnInit , ReportChildComponent {
  @Input("reportInfoObj")
  reportInfoObj: any;
  ...
  exportAsXLSX(): void {
    ...
    this.getService.getReport134(this.reportFilterForm.value,this.inputObj).subscribe((res:any) => {
        let byteArrays = res.byteData;
        utility.downloadAsExcel(byteArrays,this.reportInfoObj.reportNumber);
  });
  }
  ngOnInit(): void {
    console.log("inside reports104")
...
this.reportFilterForm.value=this.reportFilterForm.getRawValue();
    ...
    this.getService.getReport134(this.reportFilterForm.value,this.inputObj).pipe(
      finalize(() => {
        ...
        this.reportTableData = res.list.slice(0, res.list.length-3);
        // this.reportTableData = res.list;
        this.tableColumns = Object.keys(res.list[0]);
        this.summaryData = res.list[res.list.length - 3];
