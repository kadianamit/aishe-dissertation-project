...
@Component({
  selector: 'app-report148',
  templateUrl: './report148.component.html',
  styleUrls: ['./report148.component.scss']
})
export class Report148Component implements OnInit , ReportChildComponent {
...
// this.reportFilterForm.value.datePickerStart = this.datePipe?.transform(date1);
//     this.reportFilterForm.value.datePickerEnd = this.datePipe?.transform(this.reportFilterForm.value.datePickerEnd, 'dd/MM/YYYY');
    this.getService.getReport148(this.reportFilterForm.value,file).subscribe((res:any) => {
...
this.reportFilterForm.value=this.reportFilterForm.getRawValue();
    ...
    this.reportFilterForm.value.datePickerEnd = this.datePipe.transform(this.reportFilterForm.value.datePickerEnd, 'dd/MM/YYYY');
let file="JSON";
    this.getService.getReport148(this.reportFilterForm.value,file).pipe().subscribe(res => {
...
this.reportFilterForm.value=this.reportFilterForm.getRawValue();
    ...
    // this.reportFilterForm.value.datePickerStart = this.datePipe?.transform(date1);
    // this.reportFilterForm.value.datePickerEnd = this.datePipe?.transform(this.reportFilterForm.value.datePickerEnd, 'dd/MM/YYYY');
    this.getService.getReport148(this.reportFilterForm.value,file).subscribe(res => {
      let byteArrays = res.byteData;
