...
@Component({
  selector: 'app-report110b',
  templateUrl: './report110b.component.html',
  styleUrls: ['./report110b.component.scss']
})
export class Report110bComponent implements OnInit , ReportChildComponent{
 ...
 pageSize: any = 15;
  ...
  exportAsXLSX(): void {
    ...
    this.getService.getReport110B(this.reportFilterForm.value).subscribe((res:any) => {
        let byteArrays = res.byteData;
        utility.downloadAsExcel(byteArrays,this.reportInfoObj.reportNumber);
  });
  }
  ngOnInit(): void {
    console.log("inside reports110b")
  ...
  getReportDataTable(): void {
    ...
    this.getService.getReport110B(this.reportFilterForm.value).pipe(
      finalize(() => {
...
this.reportFilterForm.value=this.reportFilterForm.getRawValue();
    ...
    this.getService.getReport110B(this.reportFilterForm.value).subscribe(res => {
        let byteArrays = res.byteData;
