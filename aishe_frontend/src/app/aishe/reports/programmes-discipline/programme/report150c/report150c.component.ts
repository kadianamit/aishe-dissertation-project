...
@Component({
  selector: 'app-report150c',
  templateUrl: './report150c.component.html',
  styleUrls: ['./report150c.component.scss']
})
export class Report150cComponent implements OnInit  , ReportChildComponent {
  @Input("reportInfoObj")
  reportInfoObj: any;
  ...
  exportAsXLSX(): void {
    ...
    this.getService.getReport150C(this.reportFilterForm.value).subscribe((res:any) => {
        let byteArrays = res.byteData;
        utility.downloadAsExcel(byteArrays,this.reportInfoObj.reportNumber);
  });
  }
  ngOnInit(): void {
    console.log("inside reports150C")
  ...
  getReportDataTable(): void {
    ...
    this.getService.getReport150C(this.reportFilterForm.value).pipe(
      finalize(() => {
