...
@Component({
  selector: 'app-report150a',
  templateUrl: './report150a.component.html',
  styleUrls: ['./report150a.component.scss']
})
export class Report150aComponent implements OnInit , ReportChildComponent {
  @Input("reportInfoObj")
  reportInfoObj: any;
  ...
  exportAsXLSX(): void {
    ...
    this.getService.getReport150A(this.reportFilterForm.value).subscribe((res:any) => {
        let byteArrays = res.byteData;
        utility.downloadAsExcel(byteArrays,this.reportInfoObj.reportNumber);
  });
  }
  ngOnInit(): void {
    console.log("inside reports150A")
  ...
  getReportDataTable(): void {
    ...
    this.getService.getReport150A(this.reportFilterForm.value).pipe(
      finalize(() => {
