...
@Component({
  selector: 'app-report150',
  templateUrl: './report150.component.html',
  styleUrls: ['./report150.component.scss']
})
export class Report150Component implements OnInit , ReportChildComponent {
  @Input()
  reportInfoObj: any;
  ...
  exportAsXLSX(): void {
    ...
    this.getService.getReport150(this.reportFilterForm.value).subscribe((res:any) => {
        let byteArrays = res.byteData;
        utility.downloadAsExcel(byteArrays,this.reportInfoObj.reportNumber);
  });
  }
  ngOnInit(): void {
    console.log("inside reports150")
  ...
  getReportDataTable(): void {
    ...
    this.getService.getReport150(this.reportFilterForm.value).pipe(
      finalize(() => {
