...
@Component({
  selector: 'app-report126a',
  templateUrl: './report126a.component.html',
  styleUrls: ['./report126a.component.scss']
})
export class Report126aComponent implements OnInit, ReportChildComponent {
...
EndLimit: number = 0;
  ...
  exportAsXLSX(): void {
    ...
    this.getService.getReport126A(this.reportFilterForm.value).subscribe((res:any) => {
        let byteArrays = res.byteData;
        utility.downloadAsExcel(byteArrays,this.reportInfoObj.reportNumber);
  });
  }
  ngOnInit(): void {
    console.log("inside reports126a")
  ...
  getReportDataTable(): void {
    ...
    this.getService.getReport126A(this.reportFilterForm.value).pipe(
      finalize(() => {
...
this.reportFilterForm.value=this.reportFilterForm.getRawValue();
    ...
    this.getService.getReport126A(this.reportFilterForm.value).subscribe(res => {
        let byteArrays = res.byteData;
