...
@Component({
  selector: 'app-report126c',
  templateUrl: './report126c.component.html',
  styleUrls: ['./report126c.component.scss']
})
export class Report126cComponent implements OnInit, ReportChildComponent {
  @Input("reportInfoObj")
  reportInfoObj: any;
  ...
  exportAsXLSX(): void {
    ...
    this.getService.getReport126C(this.reportFilterForm.value).subscribe((res:any) => {
        let byteArrays = res.byteData;
        utility.downloadAsExcel(byteArrays,this.reportInfoObj.reportNumber);
  });
  }
  ngOnInit(): void {
    console.log("inside reports126c")
  ...
  getReportDataTable(): void {
    ...
    this.getService.getReport126C(this.reportFilterForm.value).pipe(
      finalize(() => {
...
this.reportFilterForm.value=this.reportFilterForm.getRawValue();
    ...
    this.getService.getReport126C(this.reportFilterForm.value).subscribe(res => {
        let byteArrays = res.byteData;
