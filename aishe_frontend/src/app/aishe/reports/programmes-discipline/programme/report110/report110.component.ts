...
@Component({
  selector: 'app-report110',
  templateUrl: './report110.component.html',
  styleUrls: ['./report110.component.scss']
})
export class Report110Component implements OnInit, ReportChildComponent {
  ...
  exportAsXLSX(): void {
    ...
    this.getService.getReport110(this.reportFilterForm.value).subscribe((res:any) => {
        let byteArrays = res.byteData;
        utility.downloadAsExcel(byteArrays,this.reportInfoObj.reportNumber);
  });
  }
  ngOnInit(): void {
    console.log("inside reports110")
  ...
  getReportDataTable(): void {
    ...
    this.getService.getReport110(this.reportFilterForm.value).pipe(
      finalize(() => {
...
this.reportFilterForm.value=this.reportFilterForm.getRawValue();
    ...
    this.getService.getReport110(this.reportFilterForm.value).subscribe(res => {
        let byteArrays = res.byteData;
