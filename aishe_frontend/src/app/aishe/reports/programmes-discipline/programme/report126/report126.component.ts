...
@Component({
  selector: 'app-report126',
  templateUrl: './report126.component.html',
  styleUrls: ['./report126.component.scss']
})
export class Report126Component implements OnInit, ReportChildComponent {
  ...
  exportAsXLSX(): void {
    ...
    this.getService.getReport126(this.reportFilterForm.value).subscribe((res:any) => {
        let byteArrays = res.byteData;
        utility.downloadAsExcel(byteArrays,this.reportInfoObj.reportNumber);
  });
  }
  ngOnInit(): void {
    console.log("inside reports126")
  ...
  getReportDataTable(): void {
    ...
    this.getService.getReport126(this.reportFilterForm.value).pipe(
      finalize(() => {
...
this.reportFilterForm.value=this.reportFilterForm.getRawValue();
    ...
    this.getService.getReport126(this.reportFilterForm.value).subscribe(res => {
        let byteArrays = res.byteData;
