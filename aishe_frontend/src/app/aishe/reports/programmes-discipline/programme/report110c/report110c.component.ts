...
@Component({
  selector: 'app-report110c',
  templateUrl: './report110c.component.html',
  styleUrls: ['./report110c.component.scss']
})
export class Report110cComponent implements OnInit, ReportChildComponent {
 ...
 pageSize: any = 15;
  ...
  exportAsXLSX(): void {
    ...
    this.getService.getReport110C(this.reportFilterForm.value).subscribe((res:any) => {
        let byteArrays = res.byteData;
        utility.downloadAsExcel(byteArrays,this.reportInfoObj.reportNumber);
  });
  }
  ngOnInit(): void {
    console.log("inside reports110c")
  ...
  getReportDataTable(): void {
    ...
    this.getService.getReport110C(this.reportFilterForm.value).pipe(
      finalize(() => {
...
this.reportFilterForm.value=this.reportFilterForm.getRawValue();
    ...
    this.getService.getReport110C(this.reportFilterForm.value).subscribe(res => {
        let byteArrays = res.byteData;
