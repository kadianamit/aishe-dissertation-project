...
@Component({
  selector: 'app-report120',
  templateUrl: './report120.component.html',
  styleUrls: ['./report120.component.scss']
})
export class Report120Component implements OnInit, ReportChildComponent {
  ...
  exportAsXLSX(): void {
    ...
    this.getService.getReport120(this.reportFilterForm.value).subscribe((res:any) => {
        let byteArrays = res.byteData;
        utility.downloadAsExcel(byteArrays,this.reportInfoObj.reportNumber);
  });
  }
  ngOnInit(): void {
    console.log("inside reports120")
  ...
  getReportDataTable(): void {
    ...
    this.getService.getReport120(this.reportFilterForm.value).pipe(
      finalize(() => {
...
this.reportFilterForm.value=this.reportFilterForm.getRawValue();
    ...
    this.getService.getReport120(this.reportFilterForm.value).subscribe(res => {
        let byteArrays = res.byteData;
