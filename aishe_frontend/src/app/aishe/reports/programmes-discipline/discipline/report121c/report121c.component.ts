...
@Component({
  selector: 'app-report121c',
  templateUrl: './report121c.component.html',
  styleUrls: ['./report121c.component.scss']
})
export class Report121cComponent implements OnInit, ReportChildComponent {
  ...
  exportAsXLSX(): void {
    ...
    this.getService.getReport121C(this.reportFilterForm.value).subscribe((res:any) => {
        let byteArrays = res.byteData;
        utility.downloadAsExcel(byteArrays,this.reportInfoObj.reportNumber);
  });
  }
  ngOnInit(): void {
    console.log("inside reports121c")
  ...
  getReportDataTable(): void {
    ...
    this.getService.getReport121C(this.reportFilterForm.value).pipe(
      finalize(() => {
...
this.reportFilterForm.value=this.reportFilterForm.getRawValue();
    ...
    this.getService.getReport121C(this.reportFilterForm.value).subscribe(res => {
        let byteArrays = res.byteData;
