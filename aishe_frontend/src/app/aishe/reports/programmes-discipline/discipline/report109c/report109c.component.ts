...
@Component({
  selector: 'app-report109c',
  templateUrl: './report109c.component.html',
  styleUrls: ['./report109c.component.scss']
})
export class Report109cComponent implements OnInit, ReportChildComponent {
  ...
  exportAsXLSX(): void {
    ...
    this.getService.getReport109C(this.reportFilterForm.value).subscribe((res:any) => {
        let byteArrays = res.byteData;
        utility.downloadAsExcel(byteArrays,this.reportInfoObj.reportNumber);
  });
  }
  ngOnInit(): void {
    console.log("inside reports109c")
  ...
  getReportDataTable(): void {
    ...
    this.getService.getReport109C(this.reportFilterForm.value).pipe(
      finalize(() => {
...
this.reportFilterForm.value=this.reportFilterForm.getRawValue();
    ...
    this.getService.getReport109C(this.reportFilterForm.value).subscribe(res => {
        let byteArrays = res.byteData;
