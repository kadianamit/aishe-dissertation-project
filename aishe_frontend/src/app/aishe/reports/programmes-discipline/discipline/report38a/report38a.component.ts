...
@Component({
  selector: 'app-report38a',
  templateUrl: './report38a.component.html',
  styleUrls: ['./report38a.component.scss']
})
export class Report38aComponent implements OnInit, ReportChildComponent {
  ...
  exportAsXLSX(): void {
    ...
    this.getService.getReport38A(this.reportFilterForm.value).subscribe((res:any) => {
        let byteArrays = res.byteData;
        utility.downloadAsExcel(byteArrays,this.reportInfoObj.reportNumber);
  });
  }
  ngOnInit(): void {
    console.log("inside reports38a")
  ...
  getReportDataTable(): void {
    ...
    this.getService.getReport38A(this.reportFilterForm.value).pipe(
      finalize(() => {
  ...
  generatePDF(): void {
    ...
    this.getService.getReport38A(this.reportFilterForm.value).subscribe(res => {
        let byteArrays = res.byteData;
