...
@Component({
  selector: 'app-report121',
  templateUrl: './report121.component.html',
  styleUrls: ['./report121.component.scss']
})
export class Report121Component implements OnInit, ReportChildComponent {
  ...
  exportAsXLSX(): void {
    ...
    this.getService.getReport121(this.reportFilterForm.value).subscribe((res:any) => {
        let byteArrays = res.byteData;
        utility.downloadAsExcel(byteArrays,this.reportInfoObj.reportNumber);
  });
  }
  ngOnInit(): void {
    console.log("inside reports121")
  ...
  getReportDataTable(): void {
    ...
    this.getService.getReport121(this.reportFilterForm.value).pipe(
      finalize(() => {
...
this.reportFilterForm.value=this.reportFilterForm.getRawValue();
    ...
    this.getService.getReport121(this.reportFilterForm.value).subscribe(res => {
        let byteArrays = res.byteData;
