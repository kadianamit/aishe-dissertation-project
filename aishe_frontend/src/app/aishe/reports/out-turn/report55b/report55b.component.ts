...
@Component({
  selector: 'app-report55b',
  templateUrl: './report55b.component.html',
  styleUrls: ['./report55b.component.scss']
})
export class Report55bComponent implements OnInit,ReportChildComponent {
  ...
  exportAsXLSX(): void {
    ...
    this.getReportService.getReport55b(this.reportFormOutTurn.value).subscribe(res => {
  ...
  ngOnInit(): void {
    console.log("inside reports55b")

  }
  getReportDataTable(): void {
    //throw new Error('Method not implemented.'); //getReport55b
   // alert('plok');
  // this.isDataLoading=true;
   this.reportFormOutTurn.value.reportType = "JSON";

   this.getReportService.getReport55b(this.reportFormOutTurn.value).pipe(
     finalize(() => {
