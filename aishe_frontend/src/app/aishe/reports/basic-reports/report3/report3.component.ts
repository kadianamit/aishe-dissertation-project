...
export class Report3Component implements OnInit, ReportChildComponent {
  ...
  exportAsXLSX(): void {
    ...
    this.reportService.getStandaloneInstituteReport(this.reportFilterForm.value).subscribe(res => {
      let bytesData = res.byteData
      
      utility.downloadAsExcel(bytesData,this.reportInfoObj.reportNumber);
    });

  }
  ngOnInit(): void {
    console.log("inside reports3")
  }
  getReportDataTable(): void {

    this.isDataLoading=true;
    this.reportFilterForm.value.reportType = "JSON";
    this.reportService.getReportStandaloneBasicData(this.reportFilterForm.value).pipe(
      finalize(() => {
