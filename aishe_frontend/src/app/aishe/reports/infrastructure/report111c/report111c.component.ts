...
@Component({
  selector: 'app-report111c',
  templateUrl: './report111c.component.html',
  styleUrls: ['./report111c.component.scss']
})
export class Report111cComponent implements OnInit,ReportChildComponent {
  ...
  exportAsXLSX(): void {
    ...
    this.reportForm.value.reportType="EXCEL";
      this.getReport.getReport111C(this.reportForm.value).subscribe(res => {
  ...
  ngOnInit(): void {
    console.log("inside reports111c")
  ...
  getReportDataTable(): void {
    ...
    this.getReport.getReport111C(this.reportForm.value).pipe(
      finalize(() => {
