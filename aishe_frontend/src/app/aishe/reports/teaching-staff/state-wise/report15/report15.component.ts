...
export class Report15Component implements OnInit, ReportChildComponent {
  ...
  exportAsXLSX(): void {
    ...
    // }
    this.reportService.getStatePostAndGenderWiseTeachingStaffpdf(this.reportFilterForm.value).subscribe(res => {
          let byteArrays = res.byteData;
          utility.downloadAsExcel(byteArrays,this.reportInfoObj.reportNumber);
    });
  }
  ngOnInit(): void {
    console.log("inside report15")
  }
...
debugger
    ...
    //     this.reportFilterForm.value.defaultState = this.stateCodeDeafult;
    // }
    this.reportService.getStatePostAndGenderWiseTeachingStaffTableData(this.reportFilterForm.value).pipe(
      finalize(() => {
