...
@Component({
  selector: 'app-report116',
  templateUrl: './report116.component.html',
  styleUrls: ['./report116.component.scss']
})
export class Report116Component implements OnInit, ReportChildComponent {
  ...
  exportAsXLSX(): void {
    ...
    this.reportService.getDistrictCategoryAndGenderWiseTeacherAllPdf(this.reportFilterForm.value).subscribe(res => {
          let byteArrays = res.byteData;
          utility.downloadAsExcel(byteArrays,this.reportInfoObj.reportNumber);
    });
  }
  ngOnInit(): void {
    console.log("inside report116")
  }
  ...
  getReportDataTable(): void {
    ...
    this.reportService.getDistrictCategoryAndGenderWiseTeachingStaffTableData(this.reportFilterForm.value).pipe(
      finalize(() => {
