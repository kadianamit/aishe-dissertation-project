...
@Component({
  selector: 'app-report116c',
  templateUrl: './report116c.component.html',
  styleUrls: ['./report116c.component.scss']
})
export class Report116cComponent implements OnInit , ReportChildComponent {
  ...
  exportAsXLSX(): void {
    ...
    this.reportService.getdistrictCategoryAndGenderWiseTeacherStandaloneInstitute(this.reportFilterForm.value).subscribe(res => {
          let byteArrays = res.byteData;
          utility.downloadAsExcel(byteArrays,this.reportInfoObj.reportNumber);
    })
  }
  ngOnInit(): void {
  console.log("inside report116c")
  }
  ...
  getReportDataTable(): void {
    ...
    this.reportService.getdistrictCategoryAndGenderWiseTeacherStandaloneInstitute(this.reportFilterForm.value).pipe(
      finalize(()=>{
