...
@Component({
  selector: 'app-report164',
  templateUrl: './report164.component.html',
  styleUrls: ['./report164.component.scss']
})
export class Report164Component implements OnInit {
   reportTableData: any;
  tableColumns: any;
  ...
  exportAsXLSX(): void {
  let file="EXCEL";
    this.getReport.getReport164(this.reportFormlistofInst.value,file).subscribe((res:any) => {
...
this.reportFormlistofInst.value=this.reportFormlistofInst.getRawValue();
   ...
   let file = "JSON";
    this.getReport.getReport164(this.reportFormlistofInst.value,file).pipe( ).subscribe((res:any) => {
