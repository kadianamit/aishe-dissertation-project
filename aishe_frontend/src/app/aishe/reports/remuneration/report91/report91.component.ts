...
@Component({
  selector: 'app-report91',
  templateUrl: './report91.component.html',
  styleUrls: ['./report91.component.scss']
})
export class Report91Component implements OnInit, ReportChildComponent {
  ...
  exportAsXLSX(): void {
    ...
    this.getService.getReport91(this.reportFilterForm.value).subscribe((res:any) => {
          let byteArrays = res.byteData;
          utility.downloadAsExcel(byteArrays,this.reportInfoObj.reportNumber);
    });
  }
  ngOnInit(): void {
    console.log("inside report91")
  ...
  getReportDataTable(): void {
    ...
    this.getService.getReport91(this.reportFilterForm.value).pipe(
      finalize(() => {
  ...
  generatePDF(): void {
    ...
    this.reportFilterForm.value.datePickerStart = this.datePipe.transform(this.reportFilterForm.value.datePickerStart, 'yyyy-MM-dd');
    this.reportFilterForm.value.datePickerEnd = this.datePipe.transform(this.reportFilterForm.value.datePickerEnd, 'yyyy-MM-dd');
    this.getService.getReport91(this.reportFilterForm.value).subscribe(res => {
      let byteArrays = res.byteData;
