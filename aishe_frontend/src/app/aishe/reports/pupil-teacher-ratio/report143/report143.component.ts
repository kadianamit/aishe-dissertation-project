...
@Component({
  selector: 'app-report143',
  templateUrl: './report143.component.html',
  styleUrls: ['./report143.component.scss']
})
export class Report143Component implements OnInit, ReportChildComponent {
  ...
  exportAsXLSX(): void {
    ...
    this.getService.getReport143(this.reportFilterForm.value).subscribe(res => {
        let byteArrays = res.byteData;
        utility.downloadAsExcel(byteArrays,this.reportInfoObj.reportNumber);
  });
  }
  ngOnInit(): void {
    console.log("inside report143")
  ...
  getReportDataTable(): void {
   ...
   // this.isDataLoading=true;
    ...
    this.getService.getReport143(this.reportFilterForm.value).pipe(
      finalize(() => {
