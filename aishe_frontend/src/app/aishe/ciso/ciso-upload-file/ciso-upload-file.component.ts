...
@Component({
  selector: 'app-ciso-upload-file',
  templateUrl: './ciso-upload-file.component.html',
  styleUrls: ['./ciso-upload-file.component.scss']
})
export class CisoUploadFileComponent implements OnInit {
  ...
  getList() {
    this.authService.uploadCisoFileList().subscribe(res => {
  ...
  uploadDocument() {
    ...
    let payload = {
      ...
      documentType: this.uploadForm.controls['documentType'].value,
      uploadedBy: this.localService.getData('userId'),
    }
    this.authService.uploadCisoFile(payload, formdata).subscribe(res => {
      if (res.status === 200) {
  ...
  searchByValue(userData: any) {
    return userData.filter((item: any) => {
      if (this.searchText.trim() === '') {
        ...
        return (item.title?.toString().trim().toLowerCase().includes(this.searchText.trim().toLowerCase()))
          || (item.documentType?.toString().trim().toLowerCase().includes(this.searchText.trim().toLowerCase()))
          || (item.dateTime?.toString().trim().toLowerCase().includes(this.searchText.trim().toLowerCase()))
          || (item.fileName?.toString().trim().toLowerCase().includes(this.searchText.trim().toLowerCase()))
      }
    })
  }
  ...
  delete(item: any) {
    this.sharedService.delete().subscribe(result => {
      if (result) {
        this.authService.deleteDocument(item.id).subscribe(res => {
          if (res.status === 200) {
