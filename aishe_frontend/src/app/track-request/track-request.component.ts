...
export class TrackRequestComponent implements OnInit{
  ...
  pendingLevelBool=false;
  systemDateTime:any;
  constructor(public fb:FormBuilder, public authService: AuthService, public sharedService: SharedService, private el: ElementRef) { 
  ...
  CheckStatus() {
    if (this.form.status == "VALID") {
      this.authService.getRequestId(this.form.value.requestId).subscribe(res => {
  ...
  setupWizard(): void {
    ...
    // console.log('55', activeElements, this.typeId)
    if (activeElements.length == 1 && this.typeId === 3) {
      this.progressBar.nativeElement.style.width = '27%';
      this.divCurtain.nativeElement.style.width = `211.3333%`;
      if (this.moeStatus == 'Pending') {
        this.progressBar.nativeElement.style.width = '75%';
        this.divCurtain.nativeElement.style.width = `0%`;
      }
      else if (this.moeStatus == 'Pending') {
        this.progressBar.nativeElement.style.width = '75%';
        this.divCurtain.nativeElement.style.width = `106%`;
      }
      else if (this.moeactiveClass != 'active') {
        this.progressBar.nativeElement.style.width = '27%';
