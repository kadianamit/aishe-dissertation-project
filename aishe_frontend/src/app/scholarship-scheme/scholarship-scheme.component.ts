...
@Component({
  selector: 'app-scholarship-scheme',
  templateUrl: './scholarship-scheme.component.html',
  styleUrls: ['./scholarship-scheme.component.scss']
})
export class ScholarshipSchemeComponent implements OnInit {
  ...
  constructor(public sharedService: SharedService, private authService: AuthService,
    public postService: PostService, private fb: FormBuilder, public getService: GetService, public router: Router, 
    public route: ActivatedRoute,public localService: LocalserviceService) {
  ...
  getScholarshipdata() {
    this.getService.scholarshipScheme(this.scholarshipForm.value, this.stateCode).subscribe((res: any) => {
  ...
  scholarshipPostdata() {
    ...
    let payload =
    {
      ...
      "stateId": this.roleId === '26' || this.roleId === '1'?this.scholarshipForm.value.stateId:this.stateCode,
      "status": true,
