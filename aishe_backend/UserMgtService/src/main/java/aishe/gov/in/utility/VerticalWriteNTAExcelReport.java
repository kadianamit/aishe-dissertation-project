...
@Component
@Log4j2
public class VerticalWriteNTAExcelReport {
    @Autowired
    ...
    public void writeNTAExcel(List<NtaQuestionEO> questions, List<NtaQuestionnaireEO> sortedCollege,
        ...
        //Workbook workbook= new XSSFWorkbook();
        List<String> question = Arrays.asList(new String[]{"State", "District", "Institution Type", "Management Type", "Ownership", "aishe code", "latitude", "longitude"});
        try (OutputStream outputStream = response.getOutputStream()) {
            ...
            Sheet hssfSheet2 = workbook.createSheet("College-Questionnaire for Eliciting NTA Centres and Other such " +
                    "CBT Exam Centres");
            workbook.getSheetAt(0).createFreezePane(Constant.firstRow, Constant.freezeRow);
            workbook.getSheetAt(1).createFreezePane(Constant.firstRow, Constant.freezeRow);
    ...
    void populateDataOnExcelFile(Sheet sheet, List<NtaQuestionnaireEO> data, List<String> questions, List<NtaQuestionEO> questionEOS, Workbook workbook) {
        ...
        //sheet.protectSheet(Constant.GOV_AISHE_IN);
        ...
        for (String question : questions) {
            ...
            excelCellOperation(row, question, data, workbook, style, subStyle, font, subFont);
        }
        for (NtaQuestionEO question : questionEOS) {
            Row row = sheet.createRow(i++);
            ntaData(row, question, data, workbook, style, subStyle, font, subFont);
        }
        log.info("File export successfully");
    }


    private void excelCellOperation(Row row, String question, List<NtaQuestionnaireEO> data, Workbook workbook, CellStyle style, CellStyle subStyle, Font font, Font subFont) {
        Integer col = Constant.ZERO;
        setHeader(workbook, col, row, style, font, question);
        if (null != data)
            for (NtaQuestionnaireEO ntaQuestionnaireEO : data) {
                col++;
                switch (question) {
                    case "State": {
    ...
    public void ntaData(Row row, NtaQuestionEO question, List<NtaQuestionnaireEO> data, Workbook workbook, CellStyle style, CellStyle subStyle, Font font, Font subFont) {
        Integer col = Constant.ZERO;
        setHeader(workbook, col, row, style, font, question.getQuestion());
        if (null != data)
            for (NtaQuestionnaireEO ntaQuestionnaireEO : data) {
                col++;
                switch (question.getQuestion_no()) {
                    case "q1": {
                    ...
                    case "q5_17": {
                        setCell(workbook, col, row, subStyle, subFont, ntaQuestionnaireEO.getQ5_17());
                        break;
                    }
                    case "q5_18": {
                        setCell(workbook, col, row, subStyle, subFont, ntaQuestionnaireEO.getQ5_18());
                        break;
                    }
                    case "q100": {
                        setCell(workbook, col, row, subStyle, subFont, ntaQuestionnaireEO.getQ100());
                        break;
                    }
                    case "q200": {
                        setCell(workbook, col, row, subStyle, subFont, ntaQuestionnaireEO.getQ200());
                        break;
                    }
                    case "q6_3": {
                        setCell(workbook, col, row, subStyle, subFont, ntaQuestionnaireEO.getQ6_3());
                        break;
                    }
                    case "q6_2": {
                        setCell(workbook, col, row, subStyle, subFont, ntaQuestionnaireEO.getQ6_2());
                        break;
                    }
                    case "q6_4": {
                        setCell(workbook, col, row, subStyle, subFont, ntaQuestionnaireEO.getQ6_4());
                        break;
                    }
                    case "q6_1": {
                        setCell(workbook, col, row, subStyle, subFont, ntaQuestionnaireEO.getQ6_1());
                        break;
                    }
                }
